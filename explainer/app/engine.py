"""
Rule-based AI explanation engine for cybersecurity alerts.

Takes an attack type, associated logs, and an IP address, then
produces a human-readable explanation, risk level, and
recommended action — without calling any external API.
"""
...
def explain(attack_type: str, logs: list[dict], ip_address: str) -> dict:
    explainer = EXPLAINERS.get(attack_type, _explain_generic)
    result = explainer(ip_address, logs)
